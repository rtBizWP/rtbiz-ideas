...
if ( ! class_exists( 'Rtbiz_Ideas_Votes_Model' ) ) {
	...
	class Rtbiz_Ideas_Votes_Model extends RT_DB_Model {
		 ...
		 * @return type
		 */
		public function get_votes_by_post( $post ) {
			global $wpdb;
			$meta_key = $post;
		...
		public function get_all_ideas() {
			global $wpdb;
			$querystr = 'SELECT *, COUNT(wvotes.vote_count) AS VOTE_COUNT FROM $wpdb->posts wposts, wp_rt_wpideas_vote wvotes' . ' WHERE wposts.ID = wvotes.post_id' . ' GROUP BY wposts.ID';
