...
function wpideas_insert_attachment( $file_handler, $idea_id, $setthumb = 'false' )
{
    ...
    if ( $setthumb ) update_post_meta( $idea_id, '_thumbnail_id', $attach_id );
...
function list_all_idea_shortcode( $atts )
{
    ...
    if ( empty( $post_type ) ) $post_type = $type;

    $post_type_ob = get_post_type_object( $post_type );
    if ( ! $post_type_ob ) return '<div class="warning"><p>No such post type <em>' . $post_type . '</em> found.</p></div>';
...
function list_woo_product_ideas( $atts )
{
    ...
    echo '</div>';
    ?><a href="javascript:;" data-nonce="<?php echo wp_create_nonce( 'load_ideas' ) ?>" id="ideaLoadMore">Load More</a>
    <input type="hidden" value="<?php echo $product_id; ?>" id="idea_product_id"/><br/><br/><?php
    if ( is_user_logged_in() ){
        ?>
        <br/>
        <div id="my-content-id" style="display:none;">
